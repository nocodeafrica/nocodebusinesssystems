...
// Mobile-optimized models (excluding large models that crash mobile browsers)
const getMobileOptimizedModels = () => {
  ...
  const allModels = [
    ...
    { 
      ...
      size: '41.38 MB',
      mobileCompatible: false // Too large for some mobile devices
    },
    { 
      id: 'loader',
      name: 'Hyundai HL975A',
      ...
      size: '74.33 MB',
      mobileCompatible: false // Too large for mobile
    },
    { 
      id: 'neighborhood',
  ...
  // On mobile, only return models compatible with mobile devices
  return isMobile ? allModels.filter(model => model.mobileCompatible) : allModels
}

const models = getMobileOptimizedModels()
...
export default function ModelViewerMobile() {
  ...
  const [modelLoadingState, setModelLoadingState] = useState<'loading' | 'loaded' | 'error'>('loading')
  ...
  useEffect(() => {
    ...
    const handleModelError = (event: any) => {
      ...
      setModelError(`Failed to load ${currentModel.name}. This model may be too large for your device.`)
      if (loadTimeoutRef.current) {
        clearTimeout(loadTimeoutRef.current)
        loadTimeoutRef.current = null
      }
    ...
    loadTimeoutRef.current = setTimeout(() => {
      if (modelLoadingState === 'loading') {
        setModelLoadingState('error')
        setModelError(`Loading timeout: ${currentModel.name} is taking too long to load. Try switching to a different model.`)
      }
    }, 30000)
  ...
  }, [currentModelIndex, isLoaded, currentModel.name, modelLoadingState])
  ...
  // Handle zoom
  useEffect(() => {
    if (modelViewerRef.current && isLoaded && modelLoadingState === 'loaded') {
      const fov = 30 - (zoomLevel * 0.3)
      modelViewerRef.current.setAttribute('field-of-view', `${fov}deg`)
    }
  }, [zoomLevel, isLoaded, modelLoadingState])
      ...
      <div className="bg-white/80 backdrop-blur-md p-4 flex items-center justify-between border-b border-gray-200">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            {isMobile && (
              <p className="text-green-600 text-xs font-medium">✓ Mobile Optimized ({currentModel.size})</p>
            )}
          </div>
        </div>
            ...
            {modelLoadingState === 'error' ? (
              <div className="flex items-center justify-center h-full bg-red-50">
                <div className="text-center p-6 max-w-sm">
                  ...
                  <h3 className="text-red-900 font-semibold text-lg mb-2">Model Load Failed</h3>
                  <p className="text-red-700 text-sm mb-4">{modelError}</p>
                ...
                {modelLoadingState === 'loading' && (
                  <div className="absolute inset-0 bg-white/80 flex items-center justify-center">
                    <div className="text-center">
                      ...
                      <p className="text-gray-700 font-medium">Loading {currentModel.name}...</p>
                      <p className="text-gray-500 text-sm mt-1">{currentModel.size}</p>
                    </div>
                  </div>
                )}
              </>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              ...
              {isMobile && (
                <div className="bg-green-50 rounded-xl p-4 border border-green-200">
                  <h3 className="text-green-900 font-medium mb-2">📱 Mobile Optimization</h3>
                  <p className="text-green-700 text-sm mb-3">
                    Large models (Hyundai Loader: 74MB, Neighborhood: 80MB) are excluded to prevent crashes and ensure smooth performance on mobile devices.
                  </p>
                  <div className="text-sm">
                    <p className="text-green-600 font-medium mb-1">Available Models:</p>
                    <ul className="space-y-1 text-green-700">
                      {models.map(model => (
                        <li key={model.id}>• {model.name} ({model.size})</li>
                      ))}
                    </ul>
                  </div>
                </div>
              ...
              <div className="bg-gray-50 rounded-xl p-4 border border-gray-200">
                ...
                <ul className="space-y-1 text-gray-600 text-sm">
                  ...
                  {isMobile && <li>• Models optimized for mobile performance</li>}
                </ul>
              </div>
            </div>
          </motion.div>
